...
public class Neurons {
    ...
    private ArrayList<Neuron> plist = new ArrayList<>();
    private double sumweight; // the weight that connects the output node to the node next to output node

    public Neurons(ArrayList inputs, int numIn, double sumweight) {
        this.numIn = numIn;
        this.inputs = inputs;
    ...
    public void buildLayer() {
        ...
        for (int i = 0; i < this.numGroup; i++) {
            ArrayList newInputs = new ArrayList(inputs.subList(i * numIn,
                                                               i * numIn + numIn));
            Neuron e = new Neuron(newInputs, this.sumweight);
            this.plist.add(e);
        }
    }

    public void generateOutputs() {
        for (Neuron e : this.plist) {
